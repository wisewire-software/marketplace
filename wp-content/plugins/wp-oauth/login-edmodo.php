...
$_SESSION['WPOA']['PROVIDER'] = 'Edmodo';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_edmodo_api_enabled'));
define('CLIENT_ID', get_option('wpoa_edmodo_api_id'));
define('CLIENT_SECRET', get_option('wpoa_edmodo_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', 'basic read_user_email'); // PROVIDER SPECIFIC: 'basic' is the minimum scope required to get the user's id from Edmodo
define('URL_AUTH', "https://api.edmodo.com/oauth/authorize?");
define('URL_TOKEN', "https://api.edmodo.com/oauth/token?");
define('URL_USER', "https://api.edmodo.com/users/me?");
...
function get_oauth_code($wpoa) {
	...
	$url = URL_AUTH . urldecode(http_build_query($params));
	header("Location: $url");
	exit;
}
...
function get_oauth_token($wpoa) {
	...
	// parse the result:
	$result_obj = json_decode($result, true); // PROVIDER SPECIFIC: Edmodo encodes the access token result as json by default
	$access_token = $result_obj['access_token']; // PROVIDER SPECIFIC: this is how Edmodo returns the access token KEEP THIS PROTECTED!
	$expires_in = $result_obj['expires_in']; // PROVIDER SPECIFIC: this is how Edmodo returns the access token's expiration
...
function get_oauth_identity($wpoa) {
	...
	$params = array(
		'access_token' => $_SESSION['WPOA']['ACCESS_TOKEN'], // PROVIDER SPECIFIC: the access_token is passed to Edmodo via POST param
	);
	$url_params = http_build_query($params);
	...
	$oauth_identity['email'] = $result_obj['email']; // PROVIDER SPECIFIC: Google returns an array of email addresses. To respect privacy we currently don't collect the user's email address.
    $oauth_identity['full_name'] = $result_obj['first_name'] .' '. $result_obj['last_name'] ;
