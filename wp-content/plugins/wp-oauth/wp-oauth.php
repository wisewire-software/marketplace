...
Class WPOA {
	...
	private $settings = array(
		...
		'wpoa_edmodo_api_enabled' => 0,									// 0, 1
		'wpoa_edmodo_api_id' => '',										// any string
		'wpoa_edmodo_api_secret' => '',									// any string
	...
	*/
	...
	function wpoa_login_user($oauth_identity) {
		...
		if(isset($oauth_identity['full_name'])){
			$_SESSION["WPOA"]["USER_FULL_NAME"] = $oauth_identity["full_name"];
		}
	...
	function wpoa_login_buttons($icon_set, $button_prefix) {
		...
		$html = "";
		$html .= $this->wpoa_login_button("google", "Sign in with Google", $atts);
		$html .= $this->wpoa_login_button("linkedin", "Sign in with LinkedIn", $atts);
		$html .= $this->wpoa_login_button("facebook", "Sign in with Facebook", $atts);
		...
		$html .= $this->wpoa_login_button("edmodo", "Sign in with Edmodo", $atts);
		if ($html == '') {
	...
	function wpoa_login_button($provider, $display_name, $atts) {
		...
		if (get_option("wpoa_" . $provider . "_api_enabled")) {
			...
			if ($atts['icon_set'] != 'none') {
				$html .= "<span class='wpoa-icon'><img src='" . $atts['icon_set_path'] . $provider . ".png' alt='" . $display_name . "' class='icon'/></span>";
			}
			$html .= $atts['button_prefix'] . " " . "<span class='wpoa-icon-text'>".$display_name."</span>";
			$html .= "</a>";
		}
		return $html;
	}
