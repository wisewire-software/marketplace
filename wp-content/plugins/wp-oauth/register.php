...
if ($_SESSION["WPOA"]["USER_ID"] != "") {
    ...
    $email = isset($_SESSION["WPOA"]["USER_EMAIL"]) ? $_SESSION["WPOA"]["USER_EMAIL"] : $username;
    $full_name = isset($_SESSION["WPOA"]["USER_FULL_NAME"]) ? $_SESSION["WPOA"]["USER_FULL_NAME"] : $username;
}
...
$update_username_result = $wpdb->update($wpdb->users, array('user_login' => $username, 'user_nicename' => $username, 'display_name' => $full_name, 'user_email' => $email), array('ID' => $user_id));
$update_nickname_result = update_user_meta($user_id, 'nickname', $username);
$update_user_email = update_user_meta($user_id, 'email', $email);
$parts_full_name = preg_split("/[\s]+/", $full_name);

if (count($parts_full_name)) {
    if (isset($parts_full_name[0])) {
        update_user_meta($user_id, 'first_name', wc_clean($parts_full_name[0]));
    }

    if (isset($parts_full_name[1])) {
        update_user_meta($user_id, 'last_name', wc_clean($parts_full_name[1]));
    }
}
