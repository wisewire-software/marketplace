<?php
/**
 * Template Name: Partner Solutions
 */

?>

<section class="section-banner_b2b">
    <?php get_header(); ?>
    <div class="container">
        <div class="jumbotron">
            <h1>Improve The Learning Experience <br>With Our Content Ecosystem & <br>Strategic Solutions</h1>
            <div class="extra-jumbotron">
                <p>Wisewire develops and designs smart and easy to use standards aligned content and technology solutions
                    customized for publishers, educators, higher education institutions, schools, and districts.</p>
                <p class="lead">Learn how we can help meet your learning product goals.</p>
            </div>
            <p class="lead">
                <a class="btn btn-primary buttons-banner btn-pink" href="#" role="button">LET'S TALK</a>
                <a class="btn btn-primary buttons-banner btn-lightblue" href="#" role="button">SCHEDULE A DEMO</a>
            </p>
        </div>
    </div>
</section>
<section class="sections_b2b">
    <div class="container">
        <h2 class="title-gray text-center">What Makes Us Different?</h2>
    </div>
    <div class="container">
        <div class="col-sm-8 content-description">
            <h3 class="title-lightblue">Our Approach</h3>
            <p class="description-b2b">
                <strong>K-12, AP:</strong>
                We have designed new-generation Learning Materials that provide
                everything needed for project-based curricula (PBL) and assessments in Math, ELA,
                Science and History.
            </p>
            <p class="description-b2b">
                <strong>College-Level Courses:</strong>
                We provide course frameworks to jumpstart development.
                Frameworks consist of competencies and objectives, content, assessments and
                learning resources.
            </p>
            <p class="description-b2b">
                Learning environments are not the same across institutions, as such our materials are
                designed to adapt to your model with the best student experience and learning-data
                fidelity.
            </p>
        </div>
        <div class="col-sm-4 content-description">
            <img class="img-responsive image-approach" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/2_WMUD.png">
        </div>
    </div>
</section>
<section class="sections_b2b bg-gray">
    <div class="container">
        <h2 class="title-orange text-center">Built For Efficient Learning</h2>
        <p class="lead text-center extra-description">
            We know that providing an engaging experience and getting <strong>reliable evidence</strong> of learning are key. Each
            learning object is designed to be an efficient bridge from the objective to the assessment.
        </p>
        <div class="content-description content-image">
            <img class="img-responsive hidden-xs" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/3_BFEL.png">
            <img class="img-responsive visible-xs-inline-block" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/3_BFEL2.png">
        </div>
    </div>
</section>
<section class="sections_b2b bg-lightblue">
    <div class="container">
        <div class="col-sm-6">
            <h2 class="title-white text-left">Case Study <span class="circle-number circle-white">1</span></h2>
            <h4 class="sub-title-white">Modifying Content To Meet Your Needs</h4>
            <p class="lead description-lightblue">
                Wisewire Playlists provide content for individual
                instruction. They provide instruction, practice and quiz for
                a standard.
            </p>
            <p class="lead description-lightblue second-description">
                We modified the playlist to for remediation by reworking
                the content to adhere to the design principles for
                remedial content.
            </p>
            <div class="lead description-lightblue">
                <div class="cont-check">
                    <img class="img-responsive mini-check" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/icon_minicheck.png">
                    <p>Simplified text and instruction</p>
                </div>
                <div class="cont-check">
                    <img class="img-responsive mini-check" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/icon_minicheck.png">
                    <p>Reduced number and complexity of practice activities</p>
                </div>
                <div class="cont-check">
                    <img class="img-responsive mini-check" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/icon_minicheck.png">
                    <p>Eliminated extensions and distractors</p>
                </div>
            </div>
        </div>
        <div class="col-sm-6 content-description">
            <img class="img-responsive image-approach" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/4_CASESTUDY_1.png">
        </div>
    </div>
</section>
<section class="sections_b2b bg-gray">
    <div class="container second-content-gray hidden-xs">
        <div class="col-sm-6 content-description">
            <img class="img-responsive" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/5_CASESTUDY_2.png">
        </div>
        <div class="col-sm-6 space-content">
            <h2 class="title-white tle-orange text-left">Case Study <span class="circle-number circle-orange">2</span></h2>
            <h4 class="sub-title-gray">Sequencing Project-Based Learning Materials</h4>
            <p class="description-b2b">
                Wisewire PBL Curriculum is a configurable basal offer in all
                core subjects.
            </p>
            <p class="description-b2b">
                <strong>Configuration 1 </strong>
                requires students to consume skill-based
                content and show mastery before applying what they learned in their final projects.
            </p>
            <p class="description-b2b">
                <strong>Configuration 2 </strong>
                tests for general readiness and then
                delivers the project. This model expects students to access the
                resources in the order they need to complete their project.
            </p>
            <p class="description-b2b">
                After completion, a final assessment provides evidence of a
                complete understanding of the standards.
            </p>
        </div>
    </div>
    <div class="container second-content-gray visible-xs-inline-block">
        <div class="col-sm-6">
            <h2 class="title-white tle-orange text-left">Case Study <span class="circle-number circle-orange">2</span></h2>
            <h4 class="sub-title-gray">Sequencing Project-Based Learning Materials</h4>
            <p class="description-b2b">
                Wisewire PBL Curriculum is a configurable basal offer in all
                core subjects.
            </p>
            <p class="description-b2b">
                <strong>Configuration 1 </strong>
                requires students to consume skill-based
                content and show mastery before applying what they learned in their final projects.
            </p>
            <p class="description-b2b">
                <strong>Configuration 2 </strong>
                tests for general readiness and then
                delivers the project. This model expects students to access the
                resources in the order they need to complete their project.
            </p>
            <p class="description-b2b">
                After completion, a final assessment provides evidence of a
                complete understanding of the standards.
            </p>
        </div>
        <div class="col-sm-6 content-description">
            <img class="img-responsive image-approach image-case-second" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/5_CASESTUDY_2.png">
        </div>
    </div>
</section>
<section class="sections_b2b bg-orange">
    <div class="container">
        <div class="col-sm-6">
            <h2 class="title-white text-left">Case Study <span class="circle-number circle-orange">3</span></h2>
            <h4 class="sub-title-white">Learning Materials that are Uniquely Yours</h4>
            <p class="lead description-lightblue">
                When your mission requires a very different approach,
                our learning science experts and engineers will build out
                materials that fulfill that mission.
            </p>
            <p class="lead description-lightblue">
                As an example, Wisewire partnered with a prominent
                publisher to develop the core elements for a tutoring
                system that uses newly created artificial intelligence (AI)
                to efficiently deliver computer-based support to college
                students.
            </p>
            <p class="lead description-lightblue">
                We are proud to report that our content scored the
                highest ratings overall.
            </p>
        </div>
        <div class="col-sm-6 space-content">
            <img class="img-responsive image-approach" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/6_CASESTUDY_3.png">
        </div>
    </div>
</section>
<section class="sections_b2b bg-white">
    <div class="container hidden-xs">
        <h2 class="title-orange title-gray-small text-center">Creating Quality Learning Materials at Scale</h2>
        <div class="col-sm-6 content-description image-quality">
            <img class="img-responsive image-approach" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/7_CQLMAS_1_2.png">
        </div>
        <div class="col-sm-6 content-description">
            <div class="sections-quality">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/7_CQLMAS_ICON_1.png">
                    <p>Our Secret</p>
                </div>
                <p class="description-b2b">
                    We have refined an approach to scale development with high
                    quality results. At the core is a network of 1600 subject and
                    learning experts that know our approach and adhere to our
                    guidelines.
                </p>
                <hr class="separator-quality">
            </div>
            <div class="sections-quality">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/7_CQLMAS_ICON_2.png">
                    <p>Learner-Centered Design</p>
                </div>
                <p class="description-b2b">
                    We believe the learning experience is as important as student
                    progress, therefore, we design for the best experience possible
                    while still optimizing for data and reports.
                </p>
                <hr class="separator-quality">
            </div>
            <div class="sections-quality">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/7_CQLMAS_ICON_3.png">
                    <p>Experts at Play</p>
                </div>
                <p class="description-b2b">
                    Our Learning Engineers, Interaction Designers, and Subject
                    Matter Experts mix ideas together to produce solutions that are
                    greater than their individual parts.
                </p>
            </div>
        </div>
    </div>
    <div class="container visible-xs">
        <h2 class="title-orange title-gray-small text-center">Creating Quality Learning Materials at Scale</h2>
        <div class="col-sm-6 content-description">
            <div class="sections-quality">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/7_CQLMAS_ICON_1.png">
                    <p>Our Secret</p>
                </div>
                <p class="description-b2b">
                    We have refined an approach to scale development with high
                    quality results. At the core is a network of 1600 subject and
                    learning experts that know our approach and adhere to our
                    guidelines.
                </p>
                <hr class="separator-quality">
            </div>
            <div class="sections-quality">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/7_CQLMAS_ICON_2.png">
                    <p>Learner-Centered Design</p>
                </div>
                <p class="description-b2b">
                    We believe the learning experience is as important as student
                    progress, therefore, we design for the best experience possible
                    while still optimizing for data and reports.
                </p>
                <hr class="separator-quality">
            </div>
            <div class="sections-quality">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/7_CQLMAS_ICON_3.png">
                    <p>Experts at Play</p>
                </div>
                <p class="description-b2b">
                    Our Learning Engineers, Interaction Designers, and Subject
                    Matter Experts mix ideas together to produce solutions that are
                    greater than their individual parts.
                </p>
            </div>
        </div>
        <div class="col-sm-6 content-description image-quality">
            <img class="img-responsive image-approach" src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/7_CQLMAS_1_2.png">
        </div>
    </div>
</section>
<section class="sections_b2b bg-gray last-section">
    <div class="container">
        <h2 class="title-orange title-gray-small text-center">What You Get With US</h2>
        <div class="col-sm-12 content-description">
            <div class="sections-quality sections-colors">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/8_WYGWU_ICON_1.png">
                    <div class="sub-content">
                        <p class="sub-t sub-t-lightblue">
                            Access to Our Learning Platform
                        </p>
                        <p class="description-b2b">
                            Our platform contains tens of thousands of common core aligned learning objects at your disposal. Everything from robust
                            technology enhanced items (TEIs), to quizzes, assessments, lessons, playlists and more!
                        </p>
                    </div>
                </div>
                <hr class="separator-quality">
            </div>
            <div class="sections-quality sections-colors">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/8_WYGWU_ICON_2.png">
                    <div class="sub-content">
                        <p class="sub-t sub-t-green">
                            Easy Setup
                        </p>
                        <p class="description-b2b">
                            Whether you are using our quality learning materials out of the box or customizing them, you will find that we make integration
                            with your LMS a breeze! Wisewire has an advanced learning environment and some of the most experienced engineers in the
                            industry. We can integrate via LTI/QTI, or customize your environment (i.e. create new elements to improve instructor
                            effectiveness, student scores, or the overall experience).
                        </p>
                    </div>
                </div>
                <hr class="separator-quality">
            </div>
            <div class="sections-quality sections-colors">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/8_WYGWU_ICON_3.png">
                    <div class="sub-content">
                        <p class="sub-t sub-t-orange">
                            We Are Your Strategic Partner
                        </p>
                        <p class="description-b2b">
                            Wisewire functions as a reliable partner that can facilitate any level of your mission, from assisting with team buy-in, team
                            communication and messaging, project planning and scoping of work, to providing thought leadership. Additionally, Wisewire
                            has over 2,500 subject matter experts ready to support custom solutions and strategies on the fly!
                        </p>
                    </div>
                </div>
                <hr class="separator-quality">
            </div>
            <div class="sections-quality sections-colors">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/8_WYGWU_ICON_4.png">
                    <div class="sub-content">
                        <p class="sub-t sub-t-pink">
                            We’re Industry Leaders & Trail Blazers
                        </p>
                        <p class="description-b2b">
                            Wisewire has been partnering with educators, learning institutions, and innovative organizations around the world for decades.
                            Our work can be found in some of the most widely used and highly regarded education products available today (including
                            online courses, open educational resources, interactive textbooks, assessments, game-based learning and many other types of
                            learning platforms).
                        </p>
                    </div>
                </div>
                <hr class="separator-quality">
            </div>
            <div class="sections-quality sections-colors">
                <div class="quality-header">
                    <img src="<?php echo get_template_directory_uri(); ?>/img/b2b_pages/8_WYGWU_ICON_5.png">
                    <div class="sub-content">
                        <p class="sub-t sub-t-gray">
                            Our Learning Sciences Experts Are Some of the Best in the World
                        </p>
                        <p class="description-b2b">
                            Our team consists of world-class learning science experts, data scientists, learning architects, interaction designers, authors and
                            subject matter experts that understand how to create meaningful learning experiences for students of all ages. Our people live
                            and breathe cognitive-psychological research of human learning!
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="sections_b2b bg-green">
    <div class="container">
        <h2 class="title-white-small text-center">Learn More About What We Offer & How We Make An
            Impact On Student Learning</h2>
        <div class="col-sm-12 text-center">
            <a href="#" class="button-last btn btn-primary">SCHEDULE A DEMO</a>
        </div>
    </div>
</section>

<?php get_footer(); ?>
