$(function () {
    ...
    $('#main-search-2').delegate('.searchform', 'submit', function (e) {
        e.preventDefault();

        if (isUrlValid($(this).find('[name="search"]').val())) {
            alert("Please enter a valid text.");
            return false;
        }

        location.href = '/explore/search/' + encodeURI($(this).find('[name="search"]').val()) + '/';
        return false;
    });
