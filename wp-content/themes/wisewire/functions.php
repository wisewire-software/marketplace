...
//$_SESSION['ww_page_visits'] = [];
//$current_user = wp_get_current_user();
//update_user_meta($current_user, 'ww_show_refer_modal', "true");
...
function custom_login(){
 ...
 if( 'wp-login.php' == $pagenow ) {
  wp_redirect('https://www.wisewire.com/');
  exit();
 }
}

global $show_refer_popup;

$show_refer_popup = false;

$ww_page_visits = [];

if (!isset($_SESSION['ww_page_visits'])) {

    $_SESSION['ww_page_visits'] = array($_SERVER['REQUEST_URI']);

} else {
    $ww_page_visits = $_SESSION['ww_page_visits'];
    if (count($ww_page_visits) < 3) {
        $slug = $_SERVER['REQUEST_URI'];
        if (!in_array($slug, $ww_page_visits)) {
            if ((strpos($slug, 'wp-json') == false) && (strpos($slug, 'wp-content') == false) && (strpos($slug, 'wp-admin') == false) && (strpos($slug, 'wp-ajax') == false)  && ($slug != "/explore") && ($slug != '/user-login/') && ($slug != "/user-login") && ($slug != "/") ) {
                $ww_page_visits[] = $slug;
            }
            $_SESSION['ww_page_visits'] = $ww_page_visits;
            if (count($ww_page_visits) == 3) {
                $show_refer_popup = true;
            }
        }
    }
}
...
if (isset($nopop)) {
    $show_refer_popup = false;
    $ww_page_visits = array_merge($_SESSION['ww_page_visits'], array("nop", "nop", "nop"));
    $_SESSION['ww_page_visits'] = $ww_page_visits;
   // echo var_dump($_SESSION['ww_page_visits']);
}
...
$log_name = date('Y-m-d') . '_wp_app.log';
...
function ww_send_refer_email_to_admin()
{

    $admin_refer_template = 
"Hi {{admin_name}},<br />".
"<br />".
"{{first_name}} {{last_name}} wanted you to know that they’ve had a great experience using <br />".
"Wisewire’s learning resources in their classroom.\n Wisewire has made tremendous traction on<br />".
" impacting learning through our custom content creation with schools and districts just like yours.<br />".
"<br />".
"A partnership with Wisewire can help you create:<br />".
"<br />".
"Project Based Learning (PBL)<br />".
"Assessments in Math, ELA, Science, History, and AP<br />".
"ELL Programs<br />".
"Instructional Design Support<br />".
"Course Development<br />".
"I’d love to schedule a short to call to show you how we have successfully revolutionized content<br />".
"creation for three leading schools.<br />".
"<br />".
"All the Best,<br />".
"Cathy Steinbrenner | VP, Business Solutions | Wisewire<br />".
"csteinbrenner@wisewire.com | (410) 800-4708<br />".
"Wisewire.com<br />".
"<br />".
           " ";


    $thank_you_template = "Thank you for helping to bring Wisewire to your school.  Our experts will be reaching out to your administrator shortly.<br />"
        . "As promised, here is your coupon code:  ReferAdmin2019 <br />"
        . "<br />"
        . "You may use this for any item on the Wisewire Marketplace. Simply input the code during checkout to have $50 taken off your order.<br />"
        . "<br />"
        . "Thank you again,<br />"
        . "<br />"
        . "The Team at Wisewire<br />";
    ...
    if ($current_user->exists()) {
        ...
        $user_first_name = $_POST['first_name_input'];
    }

    if ($current_user->exists()) {
        $user_last_name = $current_user->user_lastname;
    } else {
        $user_last_name = $_POST['last_name_input'];
    }

    if ($current_user->exists()) {
        $refer_thankyou_email = $current_user->user_email;
    } else {
        $refer_thankyou_email = $_POST['teacher_email_input'];
    }
    $headers = "Content-Type: text/html; charset=ISO-8859-1";
    wp_mail($refer_thankyou_email, "Your Coupon Code from Wisewire", $thank_you_template, $headers);


# Your template tags + replacements
    $admin_refer_row = array(
        'admin_name' => $_POST['admin_name_input'],
        'admin_lastname_input' => $_POST['admin_lastname_input'],
    ...
    $url = "/refer-your-administrator-thank-you";
$nonce = $_REQUEST['_ww_wp_nonce'];

if ( ! wp_verify_nonce( $nonce, 'refer_admin_form' ) ) {
        if (wp_redirect($url)) {
            exit;
        }
     die( 'Security check' ); 
...
//    var_dump($_POST);
    ...
    if (filter_var($admin_email_input, FILTER_VALIDATE_EMAIL)) {
        // valid emailaddress

        $email_ready = bind_to_template($admin_refer_row, $admin_refer_template);
...
if (!($current_user->exists())) {
	...
	$headers .= "Content-Type: text/html; charset=ISO-8859-1";
        wp_mail($admin_email_input, "Subject: You have been invited to Wisewire." , $email_ready, $headers);

        $hubspot = new SevenShores\Hubspot\Factory([
            'key' => '143847e0-46e8-4680-9233-87b16bdb5fb7',
            'oauth' => false, // default
            'base_url' => 'https://api.hubapi.com', // default
        ]);

        $guid = "6f4997d1-14e6-4c0d-b11c-ee154e2a959c";
        $portalId = "4385444";


        $form = ["email" => $admin_email_input,
            "firstname" => $_POST['admin_name_input'],
            "lastname" => $_POST['admin_lastname_input'],
            "referring_teachers_email_address" => $refer_thankyou_email,
            "referring_teachers_first_name" => $user_first_name,
            "referring_teachers_last_name" => $user_last_name];

        $response = $hubspot->forms()->submit($portalId, $guid, $form);

        if (!$current_user->exists()) {

            $guid = "3b306fe0-8fee-452f-a14d-f2b5ac0df95f";
            $portalId = "4385444";

            $form = ["email" => $refer_thankyou_email,
...
add_action('admin_post_nopriv_refer_admin_form', 'ww_send_refer_email_to_admin');
add_action('admin_post_refer_admin_form', 'ww_send_refer_email_to_admin');
...
function ww_add_cdn_to_attachment_image_url($image, $attachment_id, $size, $icon)
{
    if ($image[0] != "") {
        $image[0] = "https://static.wisewire.com" . wp_make_link_relative($image[0]);
    }
    return $image;
}

add_filter('wp_get_attachment_image_src', 'ww_add_cdn_to_attachment_image_url', 10, 4);

//add_filter('wpcf7_validate_text*', 'custom_anti_spam_filter', 20, 2);

function custom_anti_spam_filter($result, $tag)
{

    if ((isset($_POST['sd-education'])) && ($_POST['sd-education'] != '')) {
        $result->invalidate($tag, "An error occurred sending your information.");
    }
...
function website_login_url()
{return home_url();}

// changing the alt text on the logo to show your site name
function website_login_title()
...
add_filter('query_vars', function ($query_vars) {
    ...
    $query_vars[] = 'utm_campaign';
    $query_vars[] = 'utm_source';
    $query_vars[] = 'action';
    $query_vars[] = 'submit';
    $query_vars[] = 'cancel';
...
<p>
    <a href="edit.php?post_type=item&page=wisewire-control-panel" class="button button-primary button-hero">Go to Batch
        Uploads</a>
</p>
...
$appid = WAN_TEST_ENVIRONMENT ? "vekhwzrt" : "dG9rOjA5NjYxMjQ1X2RiYTZfNDRmMF85MmVmX2EwMTg5ODAwMjM2ZjoxOjA=";
$appkey = "";
$wp_intercom = new WP_Intercom($appidi,$appkey);
