      ...
      <div class="container">
        ...
        <div class="items<?php if ($control->search !== false) echo ' search-results'; ?>">
          ...
          <div class="row row-head">
            ...
            <div class="col-sm-6 col-headline<?php if ($control->search === false) echo ' col-uppercase'; ?>">
              <h1>
              <?php if ($control->search !== false): ?>
                Search results: <span><?php echo $control->search ?></span>
              <?php else: ?>
                <?php echo $control->discipline_object->name ?>
              <?php endif; ?>
              </h1>
              <p>
                <?php echo $control->posts_count ?> items <?php echo $control->search !== false ? 'found' : 'in this category' ?>
              ...
              <div class="options">
                ...
                <div class="rank">
                  ...
                  <select name="order_by" class="form-control select filter-order-by">
                    ...
                    <option value="most_recent" <?php echo $control->order_by === 'most_recent' ? 'selected' : '' ?>>Most recent</option>
                  </select>
            ...
            <div class="filters-overlay" id="filters-overlay">
              <div class="col-sm-3 col-nav">       
                ...
                <div class="col-headline<?php if ($control->search === false) echo ' col-uppercase'; ?>">
                  ...
                  <p>
                    <?php echo $control->posts_count ?> items <?php echo $control->search ? 'found' : 'in this category' ?>
                    <span class="ajax-loading small"></span>   
                    <?php if ( $active_filters = $control->display_active_filters() ) { ?>
                    <br>
                    <a href="?clear_filters=1" class="btn-filters-clear">Clear filters</a>
                      ...
                      <div id="filterPanel0" class="panel-collapse collapse <?php echo $control->filter_requested('gradelevel') ? 'in' : '' ?>" role="tabpanel" aria-labelledby="filters-heading-0">
                        <div class="panel-body panel-grades">
                          ...
                          <?php /*foreach ($control->filter_grades[''] as $grade_parent => $grade_parent_data): ?>
                            <li>
                              <a class="collapsed" role="button" data-toggle="collapse" data-parent="#filterGradeLevel" href="#<?php echo $grade_parent ?>Nav" aria-expanded="true" aria-controls="<?php echo $grade_parent ?>Nav"><?php echo $grade_parent_data['name'] ?></a><span><?php //echo $grade_parent_data['count'] ?></span>
                              <ul id="<?php echo $grade_parent ?>Nav" class="panel-collapse collapse <?php echo $grade_parent_data['open'] ? 'in' : '' ?>" role="tabpanel">
                  ...
                  <section class="lo-items">
                    ...
                    <?php if ($items): foreach ($items as $item): ?>
                      ...
                      <?php if ($item->type == 'item'): ?>
                            ...
                            <div class="details">
                              <h2 class="sub-discipline">
                                <?php echo !empty($item->subdiscipline) ? $item->subdiscipline : '&nbsp;'
                                           #$WWItems->get_subdiscipline($item->ID) ?>
                                <?php if (substr($item->item_preview,0,1) === 'Y'): ?>
                        ...
                        <article class="lo-item lo-item-col <?php echo $WWItems->get_color($item->item_content_type_icon) ?> <?php /*if($control->search !== false)*/ echo 'item-search'?>" onclick="location.href='/item/<?php echo $item->ID ?>/';">
                          <div class="img">
                          <?php
                          $item_main_image = get_field('item_main_image', $item->ID);
                          if ($item_main_image):
                          ...
                          <?php if($item->item_object_type != 'question') {
                              $level1_label =  !empty($item->subdiscipline) ? $item->subdiscipline : '&nbsp;';
                              $level2_label =  $item->item_content_type_icon;
                              $level3_label =  null;
                              $description_label = null;//$WWItems->get_grades($item->ID);
                              $code_label = $item->post_title;                                      
                          } else {
                          ...
                          <div class="content">
                            <div class="details">
                              ...
                              <p class="content-type">
                                <?php echo $level2_label?>
                              </p>
                              <?php if($level3_label!==null):?>
                              <p class="content-type">
                                <?php echo $level3_label  ?>
                              </p>
                              <?php endif?>
                              <p class="grade-level">
              ...
              <?php endif; ?>
                ...
                <nav>
                  ...
                  <?php if ($control->page_nr > 1): ?>
                    <li>
                      <a class="filter-page-nr" href="<?php echo $control->get_permalink() . ($control->page_nr - 1) . '/' ?>" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
                    </li>
                  <?php endif; ?>
                  ...
                  <li>
                    <a class="filter-page-nr" href="<?php echo $control->get_permalink() . ($control->page_nr + 1) . '/' ?>" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>
                  </li>
                  <?php endif; ?>
                  </ul>
                </nav>
